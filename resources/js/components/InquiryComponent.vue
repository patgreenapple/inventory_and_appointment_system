<template>
    <div>
        <div class="header mb-3">
            <h2>Inquiries</h2>
            <!-- <button class="btn btn-primary" style="width: 20%" v-on:click="addItemCategory()">Add Item Category</button> -->
        </div>
       ...
       <div class="container-fluid">
        ...
        <div class="modal fade" id="add-item-category" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
export default {
    ...
    methods : {
        ...
        getRecords() {
            axios.get('contact_us_login/getrecords').then(response => {
        ...
        store() {
            axios.post('contact_us_login/store', this.dataValues).then(response => {
        ...
        edit(id) {
            axios.get('contact_us_login/edit/' + id).then(response => {
                console.log(response)
                this.modalId = 'add-item-category';
                this.modalTitle = 'Edit Item Category';
        ...
        destroy(id) {
            this.$confirm("Are you sure?", 'Delete', 'question').then(() => {
                    axios.get('contact_us_login/destroy/' + id).then(response => {
                        this.messageBox('Success', response.data.message, 'success');
                     })
                });
        }
