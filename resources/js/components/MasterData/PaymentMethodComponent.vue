<template>
    <div>
        <div class="header mb-3">
            ...
            <button class="btn btn-primary" style="width: 20%" v-on:click="addItemCategory()">Add Item Payment Method</button>
        </div>
       ...
       <div class="container-fluid">
        ...
        <div class="modal fade" id="add-payment-method" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
export default {
    data() {
        return {
            dataValues : [],
            dataTable : [],
            columns : ['id', 'name', 'mobile_number' ,'created_at' ,'updated_at', 'action'],
            options : {
                headings : {
                    id : 'ID',
                    name : 'Name',
    ...
    methods : {
        ...
        getRecords() {
            axios.get('payment_method/getrecords').then(response => {
        ...
        addItemCategory() {
            this.initForm();
            this.modalId = 'add-payment-method';
            this.modalTitle = 'Add Payment Method';
            $('#' + this.modalId).modal('show');
        },

        store() {
            axios.post('payment_method/store', this.dataValues).then(response => {
        ...
        edit(id) {
            axios.get('payment_method/edit/' + id).then(response => {
        ...
        destroy(id) {
            this.$confirm("Are you sure?", 'Delete', 'question').then(() => {
                    axios.get('payment_method/destroy/' + id).then(response => {
                        this.messageBox('Success', response.data.message, 'success');
                     })
                });
        }
