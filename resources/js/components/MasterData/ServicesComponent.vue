<template>
    <div>
        <div class="header mb-3">
            <h2>Services</h2>
            <button class="btn btn-primary" style="width: 20%" v-on:click="addItemCategory()">Add Service</button>
        </div>
...
export default {
    ...
    methods : {
        ...
        getRecords() {
            axios.get('services/getrecords').then(response => {
        ...
        addItemCategory() {
            this.initForm();
            this.modalId = 'add-service';
            this.modalTitle = 'Add Service';
            $('#' + this.modalId).modal('show');
        },

        store() {
            axios.post('services/store', this.dataValues).then(response => {
        ...
        edit(id) {
            axios.get('services/edit/' + id).then(response => {
        ...
        destroy(id) {
            this.$confirm("Are you sure?", 'Delete', 'question').then(() => {
                    axios.get('services/destroy/' + id).then(response => {
                        this.messageBox('Success', response.data.message, 'success');
                     })
                });
        }
